...
Constants is an object mapping strings to values, where those values should be treated as constants.  Note that this makes it a pretty bad idea to have `Math` in there if the username might make use of `Math.random` and a pretty bad idea to have `Date` in there.
